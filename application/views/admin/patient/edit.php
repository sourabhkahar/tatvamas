<style>
    select option[disabled] { color: darkgray; font-weight: bold; }
</style>
                            ...
                            <div class="card-body">
                                <div class="row">
                                        ...
                                        <div class="form-group col-md-3">
                                            ...
                                            <input type="number" name="mobile_no" class="form-control" id="mobile_no" value="<?= $details->mobile_no; ?>"
                                                placeholder="Enter Mobile No" autocomplete="off" data-parsley-trigger="keyup" data-parsley-validation-threshold="1" 
    data-parsley-debounce="500" 
    data-parsley-type="digits" 
    data-parsley-minlength="10" 
    data-parsley-maxlength="10" maxlength="10" required
                                                >
                                        </div>
                                        <div class="form-group col-md-3">
...
<script type="text/javascript">
    ...
    var patient_id = '<?php echo $details->patient_id; ?>';
    $(document).ready(function () {

        console.log("appointment_date", jQuery("#appointment_date").val())
        var appointment_date = jQuery("#appointment_date").val();
        if (appointment_date!="") {
            $.ajax({
                type: "POST",
                url: site_url + "patient/getTimeSlot",
                dataType: "json",
                data: {
                    date_filter: appointment_date,
                    slot_no:'<?=$details->slot_no?>'
                },
                success: function (resp) {
                    console.log("resp",resp);
                    $("#start_time").html(resp.data);
                    // if (resp.data.length) {
                    //     var start_time = resp.data.map(function(e){
                    //         if (patient_id!=e.patient_id) {
                    //             return e.start_time;                                
                    //         }
                    //     });
                    //     console.log('start_time',start_time)
                    //     start_time.forEach(e=>{
                    //         $("#start_time option[value='"+e+"']").prop("disabled",true);
        ...
        jQuery("#appointment_date").on('change', function (e) {
            console.log("appointment_date", $(this).val())
            $.ajax({
                type: "POST",
                url: site_url + "patient/getTimeSlot",
                dataType: "json",
